...
# Distribution of Disease Metrics
...
ggsave("allfragsDismetrics.png",dpi=300)
```
```{r}
...
donorrisk/donortl
ggsave("DonorFragRisk.png",dpi=300)
```
```{r}
...
Old way: Cluster using: avg days diseased, avg frac exp healthy, and frac healthy
New way: Cluster using progression, transmission, frac diseased
...
ggsave("AllDEPGenos5clusters_STP_V090125.png",dpi=300)
```
...
ggsave("Figure3_v090125.png",dpi=300)
```
...
relative risk by sus group

```{r}
susdf<-aggregate(Removed~sus,data=GenoDFsub3,FUN=sum)
susdf$Healthy<-aggregate(Healthy~sus,data=GenoDFsub3,FUN=sum)$Healthy
susdf$Exposed<-aggregate(Exposed~sus,data=GenoDFsub3,FUN=sum)$Exposed
susdf
```
...
rrdata<-as.data.frame(RBayes$summary)[-nrow(RBayes$summary),]
rrdata$RRmed<-RBayes$median$RRISK
rrdata$SusGroup<-susdf$sus

```

```{r}
ggplot(rrdata,aes(x=RRmed,y=SusGroup,col=SusGroup))+
  geom_point()+
  geom_linerange(aes(xmax=rrdata$`97.5%`, xmin=rrdata$`2.5%`,col=SusGroup))+
  geom_vline(xintercept=1,lwd=0.7)+
  scale_x_log10(breaks=trans_breaks("log10",function(x) 10^x),labels = trans_format("log10", math_format(10^.x)))+
  theme_bw()+annotation_logticks(sides="b")+labs(y="",x="Median Relative Risk",size=12)+theme(axis.text.y = element_text(size=12))+
  scale_color_manual(values=c("red3","orange1","steelblue3","springgreen4"))

```


```{r}
susdf<-aggregate(Removed~sus,data=GenoDF,FUN=sum)
susdf$Healthy<-aggregate(Healthy~sus,data=GenoDF,FUN=sum)$Healthy
susdf$Exposed<-aggregate(Exposed~sus,data=GenoDF,FUN=sum)$Exposed
susdf
```
...
ggplot(rrdata,aes(x=RRmed,y=SusGroup,col=SusGroup))+
  ...
  scale_x_log10(breaks=trans_breaks("log10",function(x) 10^x),labels = trans_format("log10", math_format(10^.x)))+
  theme_bw()+annotation_logticks(sides="b")+labs(y="",x="Median Relative Risk",size=12)+theme(axis.text.y = element_text(size=12))+
  scale_color_manual(values=c("red3","orange1","steelblue3","springgreen4","palegreen1"))
...
ofav1819<-subset(ofdf,subset=Outplant.Date<="2020-01-01")
table(ofav1819$Unique.ID)
ofav1819[ofav1819$Multi.Locus.Genotype.ID=="WGS-53",]

ofmeans<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group*Time.Point,data=ofav1819,FUN=mean)
ofse<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group*Time.Point,data=ofav1819,FUN=function(x) sd(x)/sqrt(length(x)))
OFAV<-cbind(ofmeans,ofse$Perc.Survival)
colnames(OFAV)<-c("Geno","Sus.group","Time.Point","means","se")
OFAV
aggregate(Num.Frags.Array~Multi.Locus.Genotype.ID,data=ofav1819,FUN=sum)
```


```{r,fig.width=8}
ofmeans<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group,data=ofav1819,FUN=mean)
ofse<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group,data=ofav1819,FUN=function(x) sd(x)/sqrt(length(x)))
OFAV<-cbind(ofmeans,ofse$Perc.Survival)
...
```{r,fig.width=8}
ofmeans<-aggregate(Perc.Survival~Sus.group,data=ofav1819,FUN=mean)
ofse<-aggregate(Perc.Survival~Sus.group,data=ofav1819,FUN=function(x) sd(x)/sqrt(length(x)))
OFAV<-cbind(ofmeans,ofse$Perc.Survival)
colnames(OFAV)<-c("Sus.group","means","se")


ggplot(OFAV,aes(x=Sus.group,y=means,fill=Sus.group))+
  geom_bar(stat="identity",position=position_dodge(.9))+
  geom_errorbar(aes(ymax=means + se, ymin=means-se),width=0.2,position=position_dodge(.9))+
  scale_fill_manual(values=c("red3","orange1","steelblue3","springgreen4","palegreen2"), drop = FALSE)+
  #scale_x_discrete(limits=levels(bills.spsex$sps))+
  scale_y_continuous(breaks=seq(0,100,10),expand = c(0, 0),lim=c(0,110))+
  labs(x="",y="2018-2020 Outplant Percent Survival")+
  theme(legend.position ="bottom", legend.text=element_text(size=12), legend.title=element_blank()) + #extra to make it pretty
  theme(panel.background = element_blank())+
  #theme(text = element_text(family = "Times New Roman"))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_text(angle=45,hjust=1))
...
```{r}
ofpost19<-subset(ofdf,subset=Outplant.Date>="2020-01-01")
ofmeans<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group,data=ofpost19,FUN=mean)
ofse<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group,data=ofpost19,FUN=function(x) sd(x)/sqrt(length(x)))
OFAV<-cbind(ofmeans,ofse$Perc.Survival)
colnames(OFAV)<-c("Geno","Sus.group","means","se")
...
```{r}
ofmeans<-aggregate(Perc.Survival~Sus.group,data=ofpost19,FUN=mean)
ofse<-aggregate(Perc.Survival~Sus.group,data=ofpost19,FUN=function(x) sd(x)/sqrt(length(x)))
OFAV<-cbind(ofmeans,ofse$Perc.Survival)
colnames(OFAV)<-c("Sus.group","means","se")
...
ggplot(OFAV,aes(x=Sus.group,y=means,fill=Sus.group))+
  ...
  #scale_x_discrete(limits=levels(bills.spsex$sps))+
  scale_y_continuous(breaks=seq(0,100,10),expand = c(0, 0),lim=c(0,110))+
  labs(x="",y="2020-2023 Outplant Percent Survival")+
  theme(legend.position ="bottom", legend.text=element_text(size=12), legend.title=element_blank()) + #extra to make it pretty
  theme(panel.background = element_blank())+
  #theme(text = element_text(family = "Times New Roman"))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_text(angle=45,hjust=1))
...
ggplot(OFAV,aes(x=Geno,y=means,fill=Sus.group))+
  ...
  #scale_x_discrete(limits=levels(bills.spsex$sps))+
  scale_y_continuous(breaks=seq(0,100,10),expand = c(0, 0),lim=c(0,110))+
  labs(x="",y="2018-2023 Outplant Survival")+
  theme(legend.position ="bottom", legend.text=element_text(size=12), legend.title=element_blank()) + #extra to make it pretty
  theme(panel.background = element_blank())+
  #theme(text = element_text(family = "Times New Roman"))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_text(angle=45,hjust=1))
...
```
```{r}
ofmeans<-aggregate(Perc.Survival~Sus.group,data=ofdf,FUN=mean)
ofse<-aggregate(Perc.Survival~Sus.group,data=ofdf,FUN=function(x) sd(x)/sqrt(length(x)))
OFAV<-cbind(ofmeans,ofse$Perc.Survival)
colnames(OFAV)<-c("Sus.group","means","se")
...
ggplot(OFAV,aes(x=Sus.group,y=means,fill=Sus.group))+
  ...
  #scale_x_discrete(limits=levels(bills.spsex$sps))+
  scale_y_continuous(breaks=seq(0,100,10),expand = c(0, 0),lim=c(0,110))+
  labs(x="",y="2018-2023 Outplant Percent Survival")+
  theme(legend.position ="bottom", legend.text=element_text(size=12), legend.title=element_blank()) + #extra to make it pretty
  theme(panel.background = element_blank())+
  #theme(text = element_text(family = "Times New Roman"))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_text(angle=45,hjust=1))
...
```{r}
ofmeans<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group,data=ofdf,FUN=mean)
ofmeans<-aggregate(Perc.Survival~Sus.group,data=ofmeans,FUN=mean)
ofse<-aggregate(Perc.Survival~Sus.group,data=ofdf,FUN=function(x) sd(x)/sqrt(length(x)))
OFAV<-cbind(ofmeans,ofse$Perc.Survival)
colnames(OFAV)<-c("Sus.group","means","se")
...
ggsave("outplantSurvivalbygeno.png",dpi=300)
```
```{r}
rrdata$Geno<-rrdata$DEPgenos
rrsurvival<-merge(rrdata,OFAV,by=c("Geno"))
plot(rrsurvival$means,rrsurvival$RRmed)
```


```{r,fig.width=8,fig.height=4}
(free(outplantrisk,type="label")|outplantsurvival) +plot_annotation(tag_levels="a")& theme(plot.tag=element_text(face="bold"))
ggsave("Figure4_outplantrisksurvival.png",dpi=300)
```

```{r}
ofdf2<-ofdf
mod<-aov((Perc.Survival)~Sus.group,data=ofdf)
shapiro.test(mod$residuals)
acf(mod$residuals)
kruskal.test((Perc.Survival)~Sus.group,data=ofdf)

table(ofdf$Sus.group)

ofdf2<-subset(ofdf,subset=Sus.group!="1/1 Resistant")
ofdf2$Sus.group<-droplevels(ofdf2$Sus.group)

mod<-aov((Perc.Survival)~Sus.group,data=ofdf2)
shapiro.test(mod$residuals)
acf(mod$residuals)
kruskal.test((Perc.Survival)~Sus.group,data=ofdf2)
outplant.dunn<-dunnTest(Perc.Survival~Sus.group,ofdf2,"bonferroni")
subset(outplant.dunn$res,subset=P.adj>0.05)
```


```{r}
ofmeans<-aggregate(Perc.Survival~Multi.Locus.Genotype.ID*Sus.group,data=ofdf,FUN=mean)
ggplot(ofmeans,aes(x=Sus.group,y=Perc.Survival,col=Sus.group))+
  geom_boxplot()+
  geom_point(aes(color=Sus.group),position=position_jitterdodge(jitter.height=0,jitter.width=0.7),alpha=0.5)+
  scale_color_manual(values=c("red3","orange1","steelblue3","springgreen4","palegreen2"), drop = FALSE)+
  #scale_x_discrete(limits=levels(bills.spsex$sps))+
...
```{r}
ggplot(ofav1819,aes(x=Sus.group,y=Perc.Survival,col=Sus.group))+
  geom_boxplot()+
  geom_point()
  scale_color_manual(values=c("red3","orange1","steelblue3","springgreen4","palegreen2"), drop = FALSE)+
  #scale_x_discrete(limits=levels(bills.spsex$sps))+
  scale_y_continuous(breaks=seq(0,100,10),expand = c(0, 0),lim=c(0,110))+
  labs(x="",y="Percent Survival (epidemic)")+
  theme(legend.position ="bottom", legend.text=element_text(size=12), legend.title=element_blank()) + #extra to make it pretty
  theme(panel.background = element_blank())+
  #theme(text = element_text(family = "Times New Roman"))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_text(angle=45,hjust=1))
...
```
```{r}
ggplot(ofpost19,aes(x=Sus.group,y=Perc.Survival,col=Sus.group))+
  geom_boxplot()+
  scale_color_manual(values=c("red3","orange1","steelblue3","springgreen4","palegreen2"), drop = FALSE)+
  #scale_x_discrete(limits=levels(bills.spsex$sps))+
  scale_y_continuous(breaks=seq(0,100,10),expand = c(0, 0),lim=c(0,110))+
  labs(x="",y="Percent Survival (endemic)")+
  theme(legend.position ="bottom", legend.text=element_text(size=12), legend.title=element_blank()) + #extra to make it pretty
  theme(panel.background = element_blank())+
  #theme(text = element_text(family = "Times New Roman"))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_text(angle=45,hjust=1))
